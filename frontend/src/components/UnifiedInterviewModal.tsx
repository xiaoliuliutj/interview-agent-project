        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/50 p-4">
          <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .96 }}
            ...
            <div className="space-y-6 p-6">
              ...
              {config.showMore && <section className="grid gap-4 rounded-xl bg-slate-50 p-4 dark:bg-slate-900/40 sm:grid-cols-2"><label className="text-sm text-slate-600 dark:text-slate-300">主问题数量<input type="number" min={2} max={30} value={config.questionCount} onChange={event => config.setQuestionCount(Number(event.target.value))} className="mt-2 w-full rounded-lg border border-slate-200 p-2 dark:border-slate-700 dark:bg-slate-800 dark:text-white" /></label><label className="text-sm text-slate-600 dark:text-slate-300">面试时长（分钟）<input type="number" min={15} max={120} value={config.plannedDuration} onChange={event => config.setPlannedDuration(Number(event.target.value))} className="mt-2 w-full rounded-lg border border-slate-200 p-2 dark:border-slate-700 dark:bg-slate-800 dark:text-white" /></label><div className="flex items-center gap-2 text-xs text-slate-500"><FileStack className="h-4 w-4" />系统会按计划分配六个阶段</div></section>}
            </div>
