...
5. 使用 `docker compose ps` 和 `docker compose logs java-backend python-agent` 检查服务。

PostgreSQL 初始化脚本会创建 pgvector 扩展和双方首期表结构。生产环境应将脚本迁移到版本化 Flyway/Alembic 迁移，并使用反向代理、TLS、密钥管理和备份策略。不要把真实 `.env`、模型密钥或数据库密码写入镜像。
