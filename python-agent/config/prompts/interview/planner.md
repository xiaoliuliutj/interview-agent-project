...
Skill 约束：
{{skill_instructions}}
规划硬约束：用户提供的 questionCount 是总题量预算（包含开场题和追问），不得超过 20。PROJECT、FUNDAMENTAL、SCENARIO 的主问题上限为 4，CODING 的主问题上限为 2，单个主问题最多追问 2 次。不要在计划中生成具体问题，只给出阶段、主题和上限；实际数量由每轮“先评估、再路由”动态决定。
