...
> 用途：Interview Agent 的系统知识库基础资料，用于大模型、RAG 和 Agent 相关题目的检索、出题与参考要点核对。
>
> 整理范围：以下两个可公开读取的大模型面试专题页；已保留正文、代码与来源链接，未保留图片、站点导航、目录、评论和推广信息。
>
> 说明：本文仅用于本项目的学习与面试训练，内容著作权归原作者及原网站所有。

## 来源目录

- [RAG 大厂面试题汇总](https://notes.kamacoder.com/interview/llm/rag_interview.html)
- [Agent 大厂面试题汇总](https://notes.kamacoder.com/interview/llm/agent_interview.html)

## RAG 大厂面试题汇总

来源：[https://notes.kamacoder.com/interview/llm/rag_interview.html](https://notes.kamacoder.com/interview/llm/rag_interview.html)
...
幻觉是 RAG 项目最大的工程挑战，面试官必问（它本质是检索召回和生成忠实度的级联问题，更深一层的拆解见 [RAG 落地最难的地方在哪](https://notes.kamacoder.com/interview/llm/rag_hardest_parts_interview.html)）。
...
Agentic RAG 把 [Agent](https://notes.kamacoder.com/interview/llm/agent_interview.html) 的规划能力引入 RAG——LLM 自己判断：需要检索哪些数据源？检索结果够不够？不够就换个角度再检索。
...
还有一类问题普通 RAG 和 Agentic RAG 都吃力：实体之间的复杂关联、多跳推理、全局关系归纳。这种场景要把检索从向量切到知识图谱，具体见 [GraphRAG 与 LightRAG 大厂面试题汇总](https://notes.kamacoder.com/interview/llm/graphrag_interview.html)。
...
另外面试官很爱把 RAG 和微调放一起问"知识更新到底该用哪个、什么时候才值得微调"，这块的取舍判断可以看 [SFT、RLHF、DPO 面试详解](https://notes.kamacoder.com/interview/llm/finetuning_sft_rlhf_interview.html)。

加油

## Agent 大厂面试题汇总

来源：[https://notes.kamacoder.com/interview/llm/agent_interview.html](https://notes.kamacoder.com/interview/llm/agent_interview.html)
...
**RAG 是 Agent 工具箱里的"知识查询器"。** 面试时说出这个关系，比单纯对比两者的区别要有深度得多。RAG 本身的检索策略、混合检索、Rerank、幻觉处理是另一大块高频考点，单独展开见 [RAG 大厂面试题汇总](https://notes.kamacoder.com/interview/llm/rag_interview.html)。
...
这五个坑里，任务漂移和工具调用幻觉是最难缠的，单独展开见 [Agent 漂移与幻觉怎么解](https://notes.kamacoder.com/interview/llm/agent_drift_hallucination_interview.html) 和 [Agent 系统如何约束大模型幻觉](https://notes.kamacoder.com/interview/llm/agent_hallucination_control_interview.html)。
...
这篇是 Agent 面试的总览，几个高频追问方向都有独立深挖：生产级 Agent 怎么被框架管住看 [Harness Engineering 大厂面试题汇总](https://notes.kamacoder.com/interview/llm/harness_interview.html)，多个 Agent 怎么协作看 [多 Agent 通信与编排面试详解](https://notes.kamacoder.com/interview/llm/multi_agent_communication_interview.html)，能力怎么沉淀复用看 [Agent Skill 面试详解](https://notes.kamacoder.com/interview/llm/agent_skill_interview.html)。想看真实面试现场，可以读 [字节 Agent 开发四面面经](https://notes.kamacoder.com/interview/llm/20260506bytedance.html)。
