...
from app.common.exceptions import RagDependencyError, RagFilterUnsupported
...
class RagService:
    ...
    async def search(
        ...
        *,
        ...
        redis_key = f"python:rag:search:{hashlib.sha256(cache_key.encode('utf-8')).hexdigest()}"
        redis_cached = await self._cache.get_json(redis_key) if self._cache else None
        if isinstance(redis_cached, list):
            try:
                return [RagSearchResult.model_validate(item) for item in redis_cached]
        ...
        if self._cache:
            await self._cache.set_json(
                redis_key,
                [item.model_dump(mode="json", exclude={"chunk": {"embedding"}}) for item in results],
                ttl_seconds=max(1, self._policy.cache_ttl_seconds or 600),
            )
        return results
    ...
    async def invalidate_cache(self) -> None:
        ...
        if self._cache:
            await self._cache.delete_matching("python:rag:search:*")
