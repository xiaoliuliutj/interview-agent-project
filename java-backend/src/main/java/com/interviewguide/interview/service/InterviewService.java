...
public class InterviewService {
    ...
    public InterviewView start(String userId, StartInterviewRequest request) {
        ...
        sessionPersistence.createConfigured(
                ...
                request.interviewDirection(), difficulty);
        ...
        try {
            AgentResponse response = pythonAgentClient.initialize(new AgentInitializeRequest(
                    ...
                    new AgentInitializeRequest.CandidateSnapshot(
                            ...
                            request.interviewDirection(), request.customCategories(),
    ...
    public InterviewDetailView detail(String sessionId, String userId) {
        InterviewSessionEntity session = ownedSession(sessionId, userId);
        List<com.interviewguide.interview.domain.InterviewTurnEntity> persistedTurns =
                sessionPersistence.turns(sessionId);
        List<InterviewTurnView> indexedTurns = java.util.stream.IntStream.range(0, persistedTurns.size())
                .mapToObj(index -> {
                    var turn = persistedTurns.get(index);
                    return new InterviewTurnView(index, turn.getStage(), turn.getQuestion(), turn.getCandidateAnswer(),
                            turn.getEvaluationSummary(), turn.getScore(), turn.getCreatedAt());
                }).toList();
        return new InterviewDetailView(toView(session), indexedTurns);
    }
    ...
    public List<com.interviewguide.interview.domain.InterviewTurnEntity> turns(String sessionId) {
