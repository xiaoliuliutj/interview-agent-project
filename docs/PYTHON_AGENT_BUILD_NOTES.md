...
RAG 首期服务于项目预置的面试题库和评价知识库，由项目维护者离线导入，不开放普通候选人上传文档。候选人的简历是会话初始化输入，不是 RAG 知识库。

现有通用 `rag.index` 与由请求方传入 `knowledgeBaseIds` 的接口设计不符合该边界：后续实现题目检索时，应由下层根据 InterviewPlan 和当前阶段选择内部题库，普通上层请求不再控制知识库范围。
