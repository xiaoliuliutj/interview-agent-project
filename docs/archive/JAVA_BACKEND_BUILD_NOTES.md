...
回答接口现在返回 `InterviewDetailView`，其中同时包含更新后的会话视图和已经落库的全部可见轮次。前端每轮提交后都以该详情重建消息序列，因此面试官问题、用户回答、逐轮评估和下一题不会消失。进度展示当前阶段主问题、累计主问题、当前主问题追问、已发出题数和动态安全上限；题量不由候选人输入。

完成会话时 Java 持久化 `finalEvaluation`，历史列表和详情同时显示综合分、总结、优点、不足与建议；`GET /api/interviews/{sessionId}/export` 通过 PDFBox 导出完整问答与最终评估。PDF 服务使用 `processAllFonts` 兼容 `.ttc` 字体集合，已用中文普通字体和 TTC 字体回读测试验证。
...
- 本机已使用 IntelliJ 内置 Maven 与 JBR 21 完成 Java 测试；容器验证仍需在虚拟机执行：
