...
- 已实现先评估再路由；中间阶段至少完成 2 个主问题后才允许跳阶段，题量由下层 Agent 在最多 20 题预算内动态决定。评估摘要、当前阶段主问题数、累计主问题数、追问次数和总题数通过上层返回前端。
...
- 对话完成后保留完整问题、回答和评估消息，并在同一页面显示结束语与最终报告；PDF 导出会在显式字体不可用时尝试容器内置 CJK 字体。
- Python 测试已通过 53 项，前端 TypeScript 检查已通过；Java 容器构建需在虚拟机执行。
